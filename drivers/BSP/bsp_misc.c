...
void BSP_LED_Init(void) {
    ...
    // R,G,B LEDs are active low (original)
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_RED_PIN, GPIO_PIN_RESET); // GPIO_PIN_SET
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_GREEN_PIN, GPIO_PIN_RESET); // GPIO_PIN_SET
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_BLUE_PIN, GPIO_PIN_RESET); // GPIO_PIN_SET
...
void BSP_LED_DeInit(void){
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_RED_PIN, GPIO_PIN_SET); // GPIO_PIN_RESET
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_GREEN_PIN, GPIO_PIN_SET); // GPIO_PIN_RESET
	HAL_GPIO_WritePin(LED_GPIO_PORT, LED_BLUE_PIN, GPIO_PIN_SET); // GPIO_PIN_RESET
...
void BSP_LED_On(Led_TypeDef Led) {
  if (Led < 3)  {
     HAL_GPIO_WritePin(LED_GPIO_PORT, GPIO_PIN[Led], GPIO_PIN_SET); // GPIO_PIN_RESET
  }
}
...
void BSP_LED_Off(Led_TypeDef Led){
  if (Led < 3)  {
    HAL_GPIO_WritePin(LED_GPIO_PORT, GPIO_PIN[Led], GPIO_PIN_RESET); // GPIO_PIN_SET
  }
}
